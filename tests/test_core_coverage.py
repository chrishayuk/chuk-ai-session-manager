...
class TestInitModule:
    ...
    def test_configure_storage_failure(self):
        from chuk_ai_session_manager import configure_storage
        with patch("chuk_ai_session_manager.setup_chuk_sessions_storage", side_effect=RuntimeError("boom")):
    ...
    def test_is_available_returns_dict_with_expected_keys(self):
        ...
        expected_keys = {"core_enums", "core_models", "simple_api", "storage", "infinite_context",
                         "tool_processor", "prompt_builder", "token_tracking", "exceptions",
                         "session_manager", "redis_support", "enhanced_token_counting"}
    ...
    async def test_get_storage_info_success(self):
        ...
        mock_backend.get_stats.return_value = {"backend": "chuk_sessions", "sandbox_id": "test-sb"}
        with patch("chuk_ai_session_manager.session_storage.get_backend", return_value=mock_backend):
    ...
    async def test_get_storage_info_error(self):
        from chuk_ai_session_manager import get_storage_info
        with patch("chuk_ai_session_manager.session_storage.get_backend", side_effect=RuntimeError("storage dead")):
            info = get_storage_info()
...
class TestSimpleApiCoverage:
    ...
    async def test_track_tool_use_with_error(self):
        ...
        with patch("chuk_ai_session_manager.session_storage.get_backend", return_value=mock_store):
            with patch("chuk_ai_session_manager.models.session.Session.create") as mock_create:
                ...
                mock_create.return_value = session
                sid = await track_tool_use(tool_name="search", arguments={"query": "test"}, result=None, error="timeout")
    ...
    async def test_track_tool_use_with_session_id(self):
        ...
        with patch("chuk_ai_session_manager.session_storage.get_backend", return_value=mock_store):
            with patch("chuk_ai_session_manager.session_storage.ChukSessionsStore") as mock_css:
                store_inst = AsyncMock()
                store_inst.get.return_value = existing
                store_inst.save = AsyncMock()
                mock_css.return_value = store_inst
                sid = await track_tool_use(tool_name="weather", arguments={"location": "NYC"}, result={"temp": 72}, session_id="existing-tool-session")
...
class TestInfiniteConversationCoverage:
    ...
    async def test_process_message_triggers_segmentation(self):
        ...
        manager = InfiniteConversationManager(token_threshold=100, max_turns_per_segment=50, summarization_strategy=SummarizationStrategy.BASIC)
        with patch("chuk_ai_session_manager.infinite_conversation.get_backend", return_value=mock_store):
            with patch("chuk_ai_session_manager.infinite_conversation.ChukSessionsStore") as mock_css:
                store_inst = AsyncMock()
                ...
                with patch("chuk_ai_session_manager.models.session.Session.create") as mock_create:
                    ...
                    result_id = await manager.process_message(session_id=session.id, message="Triggers segmentation", source=EventSource.USER, llm_callback=llm_callback, model="gpt-4")
    ...
    async def test_build_context_for_llm_with_ancestor_summaries(self):
        ...
        summary_event = SessionEvent(message="Parent conversation summary: discussed weather.", source=EventSource.SYSTEM, type=EventType.SUMMARY)
        await parent.add_event(summary_event)
        sessions[parent.id] = parent
        child = Session()
        child.id = "child-session"
        child.parent_id = "parent-session"
        msg_event = SessionEvent(message="What about tomorrow?", source=EventSource.USER, type=EventType.MESSAGE)
        ...
        with patch("chuk_ai_session_manager.infinite_conversation.get_backend", return_value=mock_store):
            with patch("chuk_ai_session_manager.infinite_conversation.ChukSessionsStore") as mock_css:
                ...
                store_inst.get.side_effect = lambda sid: sessions.get(sid)
                mock_css.return_value = store_inst
                with patch("chuk_ai_session_manager.models.session.Session.ancestors", new_callable=AsyncMock, return_value=[parent]):
                    context = await mgr.build_context_for_llm(session_id="child-session", max_messages=10, include_summaries=True)
    ...
    async def test_get_session_chain(self):
        ...
        with patch("chuk_ai_session_manager.infinite_conversation.get_backend", return_value=mock_store):
            with patch("chuk_ai_session_manager.infinite_conversation.ChukSessionsStore") as mock_css:
                ...
                store_inst.get.side_effect = lambda sid: sessions.get(sid)
                mock_css.return_value = store_inst
                with patch("chuk_ai_session_manager.models.session.Session.ancestors", new_callable=AsyncMock, return_value=[root]):
...
class TestSessionManagerCoverage:
    ...
    async def test_ensure_initialized_error_path_creates_new_session(self):
        ...
        store_inst.save = AsyncMock()
        with patch("chuk_ai_session_manager.session_storage.get_backend", return_value=AsyncMock()):
            with patch("chuk_ai_session_manager.session_storage.ChukSessionsStore") as mock_css:
                mock_css.return_value = store_inst
                with patch("chuk_ai_session_manager.models.session.Session.create") as mock_create:
                    new_session = Session()
                    ...
                    mock_create.return_value = new_session
                    sm = SessionManager(session_id="some-valid-id")
        ...
        with patch("chuk_ai_session_manager.session_storage.get_backend", return_value=AsyncMock()):
            with patch("chuk_ai_session_manager.session_storage.ChukSessionsStore") as mock_css:
                ...
                with patch("chuk_ai_session_manager.models.session.Session.create") as mock_create:
                    ...
                    mock_create.return_value = new_session
                    sm = SessionManager(session_id="fail-id", infinite_context=True, system_prompt="Be helpful", metadata={"user": "test"})
                    await sm._ensure_initialized()
        ...
        with patch("chuk_ai_session_manager.session_storage.get_backend", return_value=AsyncMock()):
            with patch("chuk_ai_session_manager.session_storage.ChukSessionsStore") as mock_css:
                ...
                with patch("chuk_ai_session_manager.models.session.Session.create") as mock_create:
                    ...
                    mock_create.return_value = new_session
                    sm = SessionManager(session_id="brand-new-session", system_prompt="Test prompt", metadata={"key": "val"}, infinite_context=True)
                    await sm._ensure_initialized()
                    assert sm._initialized is True
                    assert sm._session is not None
        ...
        with patch("chuk_ai_session_manager.session_storage.get_backend", return_value=AsyncMock()):
            with patch("chuk_ai_session_manager.session_storage.ChukSessionsStore") as mock_css:
                ...
                with patch("chuk_ai_session_manager.models.session.Session.create") as mock_create:
                    ...
                    sm = SessionManager(session_id="chain-2", infinite_context=True, store=mock_store_inst)
                    await sm._ensure_initialized()
                    sm._total_segments = 2
                    sm._session_chain = ["chain-2"]
                    sm._full_conversation = [{"role": "user", "content": "msg1"}, {"role": "assistant", "content": "resp1"}]
    ...
    async def test_get_stats_infinite_exception_in_chain_load(self):
        ...
        with patch("chuk_ai_session_manager.session_storage.get_backend", return_value=AsyncMock()):
            with patch("chuk_ai_session_manager.session_storage.ChukSessionsStore") as mock_css:
                ...
                with patch("chuk_ai_session_manager.models.session.Session.create") as mock_create:
                    mock_create.return_value = session
                    sm = SessionManager(session_id="except-chain", infinite_context=True, store=mock_store_inst)
                    await sm._ensure_initialized()
        ...
        with patch("chuk_ai_session_manager.session_storage.get_backend", return_value=AsyncMock()):
            with patch("chuk_ai_session_manager.session_storage.ChukSessionsStore") as mock_css:
                ...
                with patch("chuk_ai_session_manager.models.session.Session.create") as mock_create:
                    ...
                    sm = SessionManager(session_id="lsc-child", infinite_context=True, store=mock_store_inst)
                    await sm._ensure_initialized()
        ...
        with patch("chuk_ai_session_manager.session_storage.get_backend", return_value=AsyncMock()):
            with patch("chuk_ai_session_manager.session_storage.ChukSessionsStore") as mock_css:
                ...
                with patch("chuk_ai_session_manager.models.session.Session.create") as mock_create:
                    ...
                    sm = SessionManager(session_id="lsc-missing", infinite_context=True, store=mock_store_inst)
                    await sm._ensure_initialized()
                    await sm.load_session_chain()
...
class TestAdditionalCoverage:
    ...
    async def test_init_module_get_storage_info_log_exception(self):
        from chuk_ai_session_manager import get_storage_info
        with patch("chuk_ai_session_manager.session_storage.get_backend", side_effect=Exception("total failure")):
            info = get_storage_info()
            assert "error" in info
