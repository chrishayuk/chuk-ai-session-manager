...
from chuk_ai_session_manager.models.event_source import EventSource  # noqa: E402
from chuk_ai_session_manager.models.event_type import EventType  # noqa: E402
from chuk_ai_session_manager.models.session import Session  # noqa: E402
from chuk_ai_session_manager.models.session_event import SessionEvent  # noqa: E402
from chuk_ai_session_manager.session_prompt_builder import (  # noqa: E402
    build_prompt_from_session,
)
from chuk_ai_session_manager.session_storage import (  # noqa: E402
    ChukSessionsStore,
    get_backend,
...
from chuk_tool_processor.registry import get_default_registry, initialize  # noqa: E402
...
class UnreliableLLM:
    ...
    async def chat_completion(self, messages: List[Dict], **kwargs) -> Dict:
        ...
        if self.call_count <= len(self.scenarios):
            ...
            print(
                f"   📞 LLM Call {self.call_count}: {self._describe_response(response)}"
            )
            return response
        else:
...
class LLMRetryManager:
    ...
    async def get_valid_tool_calls(
        self, llm, messages: List[Dict], processor: CustomSessionAwareToolProcessor
    ) -> tuple[Dict, List]:
        """
        ...
        for attempt in range(1, self.max_attempts + 1):
            ...
            try:
                ...
                requested_tool = (
                    tool_calls[0].get("function", {}).get("name", "unknown")
                )
                ...
                if session.events:
                    ...
                    for event in reversed(session.events):
                        if (
                            ...
                            and event.source == EventSource.LLM
                            and isinstance(event.message, dict)
                            and "attempt" in event.message
                        ):
        ...
        raise RuntimeError(
            f"Failed to get valid tool calls after {self.max_attempts} attempts"
        )
...
async def main() -> None:
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a helpful assistant. When users ask about weather, use the weather tool to get current information.",
        },
    ...
    try:
        ...
        final_response, tool_results = await retry_manager.get_valid_tool_calls(
            llm, messages, processor
        )
        ...
        for i, event in enumerate(session.events, 1):
            print(
                f"   {i}. {event.type.value}/{event.source.value}: {str(event.message)[:100]}..."
            )
        return
