    ...
    try:
        ...
        class MockSessionManager:
            ...
            async def get_stats(self):
                user_msgs = sum(
                    1 for e in self.session.events if e.source == EventSource.USER
                )
                ai_msgs = sum(
                    1 for e in self.session.events if e.source == EventSource.LLM
                )
                return {
...
async def quickstart_with_real_llm():
    ...
    if os.getenv("OPENAI_API_KEY"):
        ...
        try:
            ...
            setup_chuk_sessions_storage(
                sandbox_id="quickstart-llm-demo", default_ttl_hours=1
            )
