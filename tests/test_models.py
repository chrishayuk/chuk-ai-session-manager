...
class TestSessionEvent:
    ...
    async def test_session_event_metadata(self):
        ...
        assert await event.has_metadata("key1") is True
        ...
        assert await event.has_metadata("key1") is False
...
class TestSessionRun:
    ...
    async def test_session_run_metadata(self):
        ...
        assert await run.has_metadata("key1") is True

        await run.remove_metadata("key1")
        assert await run.has_metadata("key1") is False
...
class TestSession:
    ...
    async def test_session_add_event(self):
        ...
        event = await SessionEvent.create_with_tokens(
            message="Test message", prompt="Test prompt", model="gpt-3.5-turbo"
        )
    ...
    async def test_session_state_management(self):
        ...
        assert await session.has_state("key1") is True
        assert await session.has_state("nonexistent") is False
        ...
        assert await session.has_state("key1") is False
