...
async def customer_support_demo():
    ...
    await sm.user_says(
        "Hi, my laptop won't turn on and I have a presentation in 2 hours!"
    )
    ...
    await sm.user_says(
        "Yes, the adapter light is on but nothing happens when I press power."
    )
    await sm.ai_responds(
...
async def multi_language_demo():
    ...
    print(
        f"Conversation stats: {stats['total_messages']} messages, {stats['total_tokens']} tokens\n"
    )
