...
class EventCreate(BaseModel):
    ...
    event_type: str = Field(
        default="message", description="Event type: message, tool_call, or summary"
    )
...
class SessionCreate(BaseModel):
    ...
    parent_id: Optional[str] = Field(
        default=None, description="Parent session ID for hierarchical sessions"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        default=None, description="Session metadata properties"
    )
...
class PromptRequest(BaseModel):
    ...
    strategy: str = Field(
        default="minimal",
        description="Prompt strategy: minimal, task, tool, conversation, hierarchical",
    )
    max_tokens: Optional[int] = Field(
        default=None, description="Maximum tokens for prompt"
    )
    include_parent_context: bool = Field(
        default=False, description="Include parent session context"
    )
...
async def list_sessions(
    limit: int = Query(
        default=50, le=100, description="Maximum number of sessions to return"
    ),
...
@app.get(
    "/sessions/{session_id}", response_model=SessionResponse, summary="Get Session"
)
...
@app.post(
    "/sessions/{session_id}/events", response_model=EventResponse, summary="Add Event"
)
async def add_event(session_id: str, event: EventCreate):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid source or event type: {err}"
        )
    ...
    return EventResponse(
        ...
        token_usage=new_event.token_usage.model_dump()
        if new_event.token_usage
        else None,
    )
...
async def build_prompt(session_id: str, req: PromptRequest):
    ...
    return PromptResponse(
        prompt=prompt, token_estimate=token_estimate, strategy_used=req.strategy
    )
