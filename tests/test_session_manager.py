...
class TestMessageTracking:
    ...
    async def test_ai_response_tracking(self):
        ...
        session_id = await sm.ai_responds(response, model="gpt-4", provider="openai")
...
class TestInfiniteContext:
    ...
    async def test_get_session_chain(self):
        ...
        sm = SessionManager(infinite_context=True, max_turns_per_segment=2)
...
class TestThreadSafety:
    ...
    async def test_concurrent_messages(self):
        ...
        tasks = [sm.user_says(f"Message {i}") for i in range(5)]
    ...
    async def test_concurrent_initialization(self):
        ...
        tasks = [sm.user_says(f"Concurrent message {i}") for i in range(3)]
...
class TestEdgeCases:
    ...
    async def test_none_values(self):
        ...
        await sm.tool_used(tool_name="test", arguments={}, result=None, error="Failed")
...
class TestIntegration:
    ...
    async def test_session_recovery_after_error(self):
        ...
        _session_id = sm.session_id  # noqa: F841 - kept for documentation
