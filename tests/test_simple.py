...
class TestBasicFunctionality:
    ...
    async def test_session_manager_infinite_context(self):
        ...
        assert sm_infinite.is_infinite is True

        sm_regular = SessionManager(infinite_context=False)
        assert sm_regular.is_infinite is False

        # Test default is False
        sm_default = SessionManager()
        assert sm_default.is_infinite is False
...
class TestErrorHandling:
    ...
    async def test_none_values_handled(self):
        ...
        session_id = await sm.tool_used(tool_name="test", arguments={}, result=None)
        assert isinstance(session_id, str)
